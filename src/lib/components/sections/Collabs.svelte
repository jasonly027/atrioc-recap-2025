<script lang="ts">
  ...
  const people = stats.collabCount
    .map(([name, count]) => [startCase(name as string), count as number] as const)
    .filter(([name, count]) => count >= 4 || name === 'Lulw Master');

  const topPeople = $state(people.slice(0, 2).map(([name, count]) => ({ name, count, hidden: true })));
</script>

<Section id="collabs" nextId="games">
  <SectionHeader>Collabs</SectionHeader>

  <p class=" text-center text-xl sm:max-w-80">It's obvious who're the remaining two, but who is number one?</p>

  <table class="[&_th,td]:p-5">
    <caption class="sr-only"> Atrioc's most collabed with creators</caption>
    ...
    <tbody class="[&_tr]:bg-cgray-900 [&_tr]:odd:bg-cgray-800">
      ...
      {#each people.slice(2) as [name, count]}
        <tr class="group">
          <td class="text-left group-last:rounded-bl-lg">{name}</td>
          <td class="text-right group-last:rounded-br-lg">{count}</td>
        </tr>
